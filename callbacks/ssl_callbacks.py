...
class SSLEvalCallback(Callback):
    def __init__(
        ...
        self.test_representations = {"default": [], "first_step": [], "final_step": []}
        self.labels = []
        self.images = []
    ...
    def reinitialize_linear_evaluator(self, pl_module):
        ...
        self.linear_evaluator = LogisticRegression(
            self.z_dim, self.num_classes, learning_rate=1e-3, checkpoint_callback=False
        )
    ...
    def do_representation_eval(self, trainer, pl_module, representations, repr_type):
        ...
        print(
            f"Feature alignments: Cross-class : {cross_class_mean_cosine:.2f}   Within-class: {within_class_mean_cosine:.2f}   Ratio: {torch.abs(within_class_mean_cosine / cross_class_mean_cosine):.2f}"
        )
        if repr_type == "default":
            repr_type = ""
        else:
